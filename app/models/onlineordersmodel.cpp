...
void OnlineOrdersModel::returnOrder(const int &orderId, const int &returnAccountId, const QJsonArray items)
{
    QJsonObject params;
...
QVariant OnlineOrdersModel::data(const QModelIndex &index, int role) const
{
    ...
    if(key.m_key=="district" && role==Qt::DisplayRole){
        ...
        if(record.value("details").toString().size()){
            combo.append(QString(" - %1").arg(record.value("details").toString()));
        }
        return combo;
    }
